...
class Consolidated
{
    ...
    public function __construct(Metrics $metrics)
    {
        ...
        $avg->distance = count($packages) ? round($avg->distance / count($packages), 2): 0;
        $avg->incomingCDep = count($packages) ? round($avg->incomingCDep / count($packages), 2) : 0;
        $avg->incomingPDep = count($packages) ? round($avg->incomingPDep / count($packages), 2) : 0;
        $avg->outgoingCDep = count($packages) ? round($avg->outgoingCDep / count($packages), 2) : 0;
        $avg->outgoingPDep = count($packages) ? round($avg->outgoingPDep / count($packages), 2) : 0;
        $avg->classesPerPackage = count($packages) ? round($avg->classesPerPackage / count($packages), 2) : 0;
